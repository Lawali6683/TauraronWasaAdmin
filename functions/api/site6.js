const ALLOWED_ORIGINS = ["https://tauraronwasa.pages.dev", "http://localhost:8080"];
...
async function updateFixtures(env) {
  ...
  const response = await fetch(apiUrl, { headers: { "X-Auth-Token": env.FOOTBALL_DATA_API_KEY6 } });
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (apiKey !== REQUIRED_API_KEY) return new Response(JSON.stringify({ error: true, message: "Invalid API Key" }), { status: 401, headers });
