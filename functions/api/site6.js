...
async function updateFixtures(env) {
    ...
    const metaRes = await fetch(metaUrl, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ lastLoadTime: now })
    });
    
    
    if (!metaRes.ok) {
        console.error("Kuskure wajen sabunta Firebase Metadata.");
    }
...
export async function onRequest({ request, env }) {
    ...
    try {
        ...
        console.error("Global Request Error:", error.message);
